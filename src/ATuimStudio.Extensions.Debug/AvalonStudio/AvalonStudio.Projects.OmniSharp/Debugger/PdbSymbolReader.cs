...
namespace AvalonStudio.Debugging.DotNetCore
{
	...
	public class PdbSymbolReader : ISymbolReader, ISymbolReader2
	{
		...
		private bool LoadPdbFile(string pdbFileName)
		{
			...
			using (var metadataReader = MetadataReaderProvider.FromPortablePdbStream(fs))
			{
				...
				foreach (var documentHandle in pdb.Documents)
				{
					...
					if (!methodMapping.TryGetValue(documentHandle, out List<SymbolMethod> list))
						list = new List<SymbolMethod>();

					var document = pdb.GetDocument(documentHandle);
					string fullFilePath = pdb.GetString(document.Name);
					SourceFileDebugInfo info = new SourceFileDebugInfo(list)
					{
						Hash = pdb.GetBlobBytes(document.Hash),
						HashAlgorithm = pdb.GetGuid(document.HashAlgorithm),
						FileID = 0,
					...
					fileToSourceFileInfos[fullFilePath] = new List<SourceFileDebugInfo> { info };
		...
		public ISymbolMethod GetMethodFromDocumentPosition(ISymbolDocument document, int line, int column)
		{
			...
			if (matchingDocument != null)
			{
				...
				var matching = sequencePoints.Where(item => item.sp.IsWithin(line, column)).OrderBy(item => item.sp.LineRange());
		...
		{
			IEnumerable<SourceFileDebugInfo> matchingDocuments = sourceFilesDebugInfo.SelectMany(static doc => doc.Value)
				.SelectMany(static sf => sf.Value).Where(sf => sf.URL == document.URL);

			IEnumerable<(SymbolMethod method, SequencePoint sp)> sequencePoints = matchingDocuments
				.SelectMany(doc => doc.PdbMethods.SelectMany(static m => m.SequencePoints, (method, sp) => (method, sp)));

			IEnumerable<(SymbolMethod method, SequencePoint sp)> matching = sequencePoints.Where(item => item.sp.IsWithin(line, column)).OrderBy(static item => item.sp.LineRange());

			return [.. matching.Select(static x => x.method).DistinctBy(static x => x.Token)];
		}
