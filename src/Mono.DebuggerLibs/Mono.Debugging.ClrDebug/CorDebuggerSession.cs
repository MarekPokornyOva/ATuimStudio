...
namespace Mono.Debugging.ClrDebug
{
	public class CorDebuggerSession : DebuggerSession
	{
			...
			lock (terminateLock)
			{
				...
				if (process != null)
				{
					...
					catch (COMException e)
					{
						...
						if ((HRESULT)e.ErrorCode != HRESULT.CORDBG_E_PROCESS_TERMINATED)
						{
							throw;
						}
					}
		...
		protected override void OnRun(DebuggerStartInfo startInfo)
		{
			MtaThread.Run(delegate
			{
				...
				if (!startInfo.UseExternalConsole)
				{
					...
					flags |= CreateProcessFlags.PROFILE_SERVER; //DebuggerExtensions.CREATE_REDIRECT_STD
				}
		...
		void OnModuleLoad(object sender, CorModuleEventArgs e)
		{
			...
			CorMetadataImport mi = new CorMetadataImport(currentModule.CreateMetaDataImport());
			...
			if (file.IndexOfAny(badPathChars) == -1)
			{
				try
				{
					if (CustomSymbolReaderFactory != null)
					...
					else
					{
						...
						foreach (var met in methods)
						{
							foreach (SequencePoint sp in met.GetSequencePoints())
							{
								...
								{  //breakpoint is inside current sequence point
									if (bestSp == null || bestSp.IsInside(doc.Document.URL, sp.StartLine, sp.StartColumn))
									{  //and sp is inside of current candidate
										bestSp = sp;
										bestMethod = met;
										break;
									}
											  ...
											  && sp.StartColumn <= bp.Column)
								{  //breakpoint is on the same line and on the right side of sp
								...
								{  //sp is after bp
									if (bestRightSideSp == null
										 ...
										 || (bestRightSideSp.StartLine == sp.StartLine && bestRightSideSp.StartColumn > sp.StartColumn))
									{ //and current candidate is on the right side of it
										bestRightSideSp = sp;
		...
		void Step(bool into)
		{
			try
			{
				...
				if (stepper != null)
				{
					...
					frame.GetIP(out int offset, out CorDebugMappingResult mappingResult);
		...
		{
			if (stepOverAll)
				stepper.StepRange(into, new[] { new COR_DEBUG_STEP_RANGE() { startOffset = 0, endOffset = int.MaxValue } });
			else
				stepper.Step(into);
			ClearEvalStatus();
			process.Continue(false);
	...
	static class SequencePointExt
	{
		...
		public static Type GetTypeInfo(this CorType type, CorDebuggerSession session)
		{
			...
			if (type.Type == CorElementType.Array || type.Type == CorElementType.SZArray)
			{
