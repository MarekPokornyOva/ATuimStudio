...
namespace Microsoft.Samples.Debugging.CorMetadata
{
    ...
    {
        internal MetadataType(IMetaDataImport importer,mdToken classToken)
        {
            ...
            else
            {
                ...
                importer.GetTypeDefProps((mdTypeDef)classToken,
                                         null,
                                         0,
                                         out size,
                                         out pdwTypeDefFlags,
				...
				if (size == 0) {
					mdToken ptkResScope;
					importer.GetTypeRefProps ((mdTypeRef)classToken,
						out ptkResScope,
						null,
						0,
						out size
                ...
                m_name = GetNestedClassPrefix(importer,classToken,(TypeAttributes)pdwTypeDefFlags) + szTypedef.ToString();
                ...
                if (baseTypeName == "System.Enum")
                {
                    ...
                    int sigSize;
                    ppvSig = IntPtr.Zero;
                    int hr = (int)importer.GetCustomAttributeByName(classToken,"System.FlagsAttribute",out ppvSig,out sigSize);
                    if (hr < 0)
                    {
        ...
        private static string GetTypeName(IMetaDataImport importer, int tk)
        {
                ...
                mdToken token = tk;
                if (token.Type==CorTokenType.mdtTypeDef)
                {
                    importer.GetTypeDefProps((mdTypeDef)tk,
                                        null,
                                        0,
                                        out size,
                                        out _,
						  ...
						  sbBaseName = new StringBuilder(size);
                    importer.GetTypeDefProps((mdTypeDef)tk,
                                        sbBaseName,
                                        sbBaseName.Capacity,
                                        out size,
                ...
                return sbBaseName==null ? "" : sbBaseName.ToString();
        }

        private static CorElementType GetEnumUnderlyingType(IMetaDataImport importer, int tk)
        {
                IntPtr hEnum = IntPtr.Zero;
                mdFieldDef[] mdFieldDef = [new mdFieldDef()];
                int numFieldDefs;
                CorFieldAttr fieldAttributes;
                int nameSize;
                CorElementType cPlusTypeFlab;
                IntPtr ppValue;
                int pcchValue;
                IntPtr ppvSig;
                int size;
                ...
                importer.EnumFields(ref hEnum, tk, mdFieldDef, 1, out numFieldDefs);
                while (numFieldDefs != 0)
                {
                    importer.GetFieldProps(mdFieldDef[0],out classToken,null,0,out nameSize,out fieldAttributes,out ppvSig,out size,out cPlusTypeFlab,out ppValue,out pcchValue);
        ...
        public override Type BaseType 
        {
            get 
            {
				...
				m_importer.GetTypeDefProps ((mdTypeDef)m_typeToken,
				                            null,
				                            0,
				                            out _,
				                            out _,
				...
				if (ptkExtends == 0 || ptkExtends.Rid == 0)
					return null;
        ...
        public override String FullName 
        {
            get 
            {
				System.Text.StringBuilder sb = new System.Text.StringBuilder (m_name);
				if (m_typeArgs != null) {
					sb.Append ("[");
        ...
        public override PropertyInfo[] GetProperties(BindingFlags bindingAttr)
        {
			...
			mdProperty[] methodToken = [new mdProperty()];
			try {
				while (true) {
					int size;
					m_importer.EnumProperties (ref hEnum, (mdTypeDef)m_typeToken, methodToken, 1, out size);
					if (size == 0)
						break;
        ...
        public override Type[] GetInterfaces()
        {
			...
			mdInterfaceImpl[] impl = [new mdInterfaceImpl()];
			try 
			{
				while(true)
				{
					int size;
					m_importer.EnumInterfaceImpls (ref hEnum,(mdTypeDef)m_typeToken, impl,1,out size);
					if(size==0)
						break;
					mdTypeDef classTk;
					mdToken intfTk;
        ...
        public override FieldInfo[] GetFields(BindingFlags bindingAttr)
        {
            ...
            mdFieldDef[] fieldToken = [new mdFieldDef()];
            try 
            {
                while(true)
                {
                    int size;
                    m_importer.EnumFields(ref hEnum,(mdTypeDef)m_typeToken, fieldToken,1,out size);
                    if(size==0)
                        break;
					// [Xamarin] Expression evaluator.
        ...
        public override MethodInfo[] GetMethods(BindingFlags bindingAttr)
        {
            ...
            mdMethodDef[] methodToken = [new mdMethodDef()];
            try 
            {
                while(true)
                {
                    int size;
                    m_importer.EnumMethods(ref hEnum,(mdTypeDef)m_typeToken,methodToken,1,out size);
                    if(size==0)
                        break;
					// [Xamarin] Expression evaluator.
    ...
    class TypeDefEnum : IEnumerable<Type>, IEnumerator<Type>, IDisposable
    {
        ...
        {
            mdTypeDef[] token = [new mdTypeDef()];
            int c;
            
            m_corMeta.m_importer.EnumTypeDefs(ref m_enum,token,1, out c);
