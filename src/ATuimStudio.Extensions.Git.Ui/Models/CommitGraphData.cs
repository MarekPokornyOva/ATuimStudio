...
namespace ATuimStudio.Extensions.Git.Models
{
	public sealed class GraphCommit
	{
		public GraphCommit(ICommit source)
		{
			Source = source;
		}
		...
		public bool IsCurrentHead => Source.IsHead;

		IReadOnlyList<GraphCommit>? _parents;
		public IReadOnlyList<GraphCommit> Parents => _parents ??= [.. Source.Parents.Select(static x => new GraphCommit(x))];
	...
	public sealed class CommitGraphData
	{
		...
		private CommitGraphData(IReadOnlyList<Path> paths, IReadOnlyList<Link> links, IReadOnlyList<Dot> dots)
		{
			Paths = paths;
			Links = links;
			Dots = dots;
		...
		public static void SetPens(Color[] colors, double thickness)
		{
			Pens.Clear();
			...
			s_penCount = colors.Length;
		}

		public sealed class Path(int color, bool isMerged)
		{
			internal List<Point> PointsEdit = [];
			public IReadOnlyList<Point> Points => PointsEdit;
		...
		public static CommitGraphData Parse(IEnumerable<ICommit> commits, bool firstParentOnlyEnabled)
		{
			...
			GraphCommit[] graphCommits = [.. commits.Select(static x => new GraphCommit(x))];
			foreach (var commit in graphCommits)
			{
				...
				foreach (var l in unsolved)
				{
					if (l.Next.Equals(commit.SHA, StringComparison.Ordinal))
					{
						if (major == null)
						{
							...
							{
								major.Next = commit.Parents[0].SHA;
				...
				if (major == null)
				{
					...
					if (commit.Parents.Count > 0)
					{
						major = new PathHelper(commit.Parents[0].SHA, isMerged, colorPicker.Next(), new Point(offsetX, offsetY));
						unsolved.Add(major);
				...
				if (!firstParentOnlyEnabled)
				{
					...
					{
						var parentHash = commit.Parents[j].SHA;
			...
			{
				var path = unsolved[i];
				var endY = (graphCommits.Length - 0.5) * unitHeight;
			...
			return new CommitGraphData(paths, links, dots);
		}
		...
		private class PathHelper
		{
			...
			public void ReplaceMerged()
			{
				...
				Path.PointsEdit.Add(new Point(LastX, _lastY));
				_endY = 0;
			}
			...
			private void Add(double x, double y)
			{
				if (_endY < y)
				{
					Path.PointsEdit.Add(new Point(x, y));
					_endY = y;
				}
			}
		...
		private static readonly Color[] s_defaultPenColors =
		[
			Colors.Orange,
			Colors.ForestGreen,
			Colors.Turquoise,
